...
#     if min_val == max_val:
#         return spark_chars[0] * len(clean_values)
#     def normalize(v):
#         return int((v - min_val) / (max_val - min_val) * (len(spark_chars) - 1))
#     return ''.join([spark_chars[normalize(v)] for v in clean_values])
...
#         df['points'] = df['point'].astype(str)
#     else:
#         st.warning("No 'point' field found in the data.")
#         df['points'] = "Unknown"
...
#         trend_lookup = recent_data.groupby('points')['value'].apply(
#             lambda x: generate_sparkline(x.sort_index())
#         ).to_dict()

#         filtered['trend'] = filtered['points'].map(trend_lookup).fillna("No trend")

#         filtered['hover_text'] = (
#             "<b>Point:</b> " + filtered['points'] + "<br>"
#             + "<b>Description:</b> " + filtered['description'].astype(str) + "<br>"
#             + "<b>Status:</b> " + filtered['value'].map({1: "Positive", 0: "Negative"}).fillna("Unknown") + "<br>"
#             + "<b>Trend:</b> " + filtered['trend'] + "<br>"
#             + "<b>X:</b> " + filtered['x'].astype(str) + "<br>"
