...
# ---- Admin Tool to Update Coordinates ----
st.subheader("🛠️ Admin Tool: Update X, Y Coordinates for a Location")

existing_codes = df['location_code'].dropna().unique()
selected_code = st.selectbox("Select Location Code to Update", sorted(existing_codes))

with st.form("update_coordinates_form"):
    ...
    submit = st.form_submit_button("Update Coordinates")
    ...
    if submit:
        ...
        st.success(f"✅ Updated {result.modified_count} records for location_code = '{selected_code}'.")
...
if selected_date:
    ...
    # --- Last 28 days positivity analysis ---
    positivity_ratio = (
        recent_data.groupby("points")["value"]
        .agg(lambda vals: np.mean(vals.dropna()) if not vals.dropna().empty else np.nan)
    )
