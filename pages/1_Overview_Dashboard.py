...
col1, col2, col3 = st.columns(3)
col1.metric("Total Samples", len(df))
col2.metric("Detected", df[df["test_result"].str.lower() != "not detected"].shape[0])
col3.metric("Detection Rate", f"{(df[df['test_result'].str.lower() != 'not detected'].shape[0] / len(df)) * 100:.1f}%")

# 📈 Detection Over Time
detection_df = df.copy()
detection_df["test_result"] = detection_df["test_result"].astype(str).str.strip().str.lower()
detection_df["Detection"] = detection_df["test_result"].map(
    lambda v: "Detected" if v != "not detected" else "Not Detected"
)
